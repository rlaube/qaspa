...
class MLP(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, num_layers, dropout, batch_norm=False,
                 init_last_layer_bias_to_zero=False, layer_norm=False, activation='gelu', score_mlp=False):
        super().__init__()
        ...
        for i in range(self.num_layers + 1):
            ...
            if not (score_mlp and i == self.num_layers):
...
class MLP_Residual1(nn.Module):
    ...
    ----------
    num_layers: number of hidden layers
    skip_placement: 0 = add skip connection after linear (ff) layer; 1 = add skip connection after layernorm; 2 = add skip connection after activation
    """
    activation_classes = {'gelu': GELU, 'relu': nn.ReLU, 'tanh': nn.Tanh}

    def __init__(self, input_size, hidden_size, output_size, num_layers, dropout, batch_norm=False,
                 init_last_layer_bias_to_zero=False, layer_norm=False, activation='gelu', score_mlp=False, skip_placement=0):
        super().__init__()
        ...
        for i in range(self.num_layers + 1):
            ...
            if not (score_mlp and i == self.num_layers):
                ...
                if self.layer_norm:
                    self.layers.add_module(f'{i}-LayerNorm', nn.LayerNorm(self.hidden_size))
                self.layers.add_module(f'{i}-{activation}', self.activation_classes[activation]())
        if init_last_layer_bias_to_zero:
            self.layers[-1][-1].bias.data.fill_(0)

    def forward(self, x):
        identity = x
        for i, layer in enumerate(self.layers):
            if i == len(self.layers) - 4 and self.skip_placement == 0: # add connection after last linear layer
                assert(isinstance(layer, nn.Linear))
                x = layer(x) + identity
            elif i == len(self.layers) - 2 and self.skip_placement == 1: # add connection after last layernorm layer
                assert(isinstance(layer, nn.LayerNorm))
                x = layer(x) + identity
            elif i == len(self.layers) - 1 and self.skip_placement == 2: # add connection before last activation layer
                assert(isinstance(layer, self.activation_classes[self.activation]))
                x = layer(x) + identity
            else:
                x = layer(x)
        return x
    
class MLP_Residual2(nn.Module):
    """
    Multi-layer perceptron
    ...
    ----------
    num_layers: number of hidden layers
    skip_type: 0 = no skip connections, 1 = skip connection from beginning to end, 2 = skip from beginning to each layer, 3 = skip between each layer
    skip_placement: 0 = add skip connection after linear (ff) layer; 1 = add skip connection after layernorm; 2 = add skip connection after activation
    """
    activation_classes = {'gelu': GELU, 'relu': nn.ReLU, 'tanh': nn.Tanh}
    ...
    def __init__(self, input_size, hidden_size, output_size, num_layers, dropout, batch_norm=False,
        ...
        for i in range(self.num_layers + 1):
            ...
            if not (score_mlp and i == self.num_layers):
                ...
                if self.layer_norm:
                    self.layers.add_module(f'{i}-LayerNorm', nn.LayerNorm(self.hidden_size))
                self.layers.add_module(f'{i}-{activation}', self.activation_classes[self.activation]())
        if init_last_layer_bias_to_zero:
            self.layers[-1][-1].bias.data.fill_(0)
    ...
    def forward(self, x):
        identity = x
        for i, layer in enumerate(self.layers):
            if self.skip_placement == 0 and isinstance(layer, nn.Linear):
                x = layer(x) + identity
            elif self.skip_placement == 1 and isinstance(layer, nn.LayerNorm):
                x = layer(x) + identity
            elif self.skip_placement == 2 and isinstance(layer, self.activation_classes[self.activation]):
                x = layer(x) + identity
            else:
                x = layer(x)
        return x

class MLP_Residual3(nn.Module):
    """
    Multi-layer perceptron
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            # update skip connection value after each activation layer
            if isinstance(layer, self.activation_classes[self.activation]):
                identity = x
        return x
